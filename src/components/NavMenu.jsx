...
export default function NavMenu({ menu }) {
    ...
    //to open the active nav menu when redirect from another page other than /adm
    useEffect(() => {
    ...
    return (
        <div>
            ...
            {/* menu title */}
            <span onClick={(e) => { handleMenuClick(e) }} className={location.pathname.includes(menu.title) ? 'text-blue-500 pl-8 space-x-2' : 'pl-8 space-x-2'}>

                {/* icons and title */}
                <i className={menu.titleIcon}></i>
...

                {
                    ...
                    <i className={isOpen ? 'fa-solid fa-chevron-up fa-xs' : 'fa-solid fa-chevron-down fa-xs'}></i>
